...
instance Default AppOpts where
  def = AppOpts (Just 3000) (Just "http://localhost") Nothing Nothing
...
main = do
  ...
  where
    ...
    opts = info (helper <*> app)
     ...
     <> header "lifeline - a Haskell web server" )
...
entry port env = do
  ...
  run port $
      gzip def
    $ logStdoutDev
    $ runMiddlewareT (flip runApp env) application
    $ defApp
  where
    application = securityLayer . staticLayer . contentLayer
